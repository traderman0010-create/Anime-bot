...
def start(message):
    ...
    if anime_kodi in animelar:
        if check_sub(user_id):
            ...
            bot.send_video(user_id, video['id'], thumb=THUMBNAIL, caption=f"🎬 {video['caption']}\n📢 Asosiy kanal: {KANAL_ID}", protect_content=True)
        else:
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("Animelar - Uzbek Tilida✅", url=f"https://t.me/{KANAL_ID[1:]}"),
                types.InlineKeyboardButton("Mahfiy kanal✅", url=MAXFIY_KANAL),
                types.InlineKeyboardButton("Instagram✅", url=INSTAGRAM),
                types.InlineKeyboardButton("🔄 Tekshirish 🔄", callback_data=f"check_{anime_kodi}")
            )
...
def callback_check(call):
    ...
    if check_sub(call.from_user.id):
        ...
        bot.send_video(call.from_user.id, video['id'], thumb=THUMBNAIL, caption=f"🎬 {video['caption']}\n📢 Asosiy kanal: {KANAL_ID}", protect_content=True)
    else:
