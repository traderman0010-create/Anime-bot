...
KANAL_ID = '@Anivoxuz'
ANINEWS = 'https://t.me/AniVoxnews'
KANAL = 'https://t.me/+oxPibE-r26c2NjM6'
...
animelar = {
    ...
    'jahannam13': {'id': 'BAACAgIAAxkBAAIE92mgHSslGuS-xyTvvN-m7lOMXu1zAAIOjgACBt8BScGlXDDsjwdoOgQ', 'caption': 'Jahannam Jannati [13-Qism - 1-Fasl Final]'},
...
def show_sub_menu(user_id, param):
    ...
    btn1 = types.InlineKeyboardButton("Animelar - Uzbek Tilida⛩️✅", url=f"https://t.me/{KANAL_ID[1:]}")
    btn2 = types.InlineKeyboardButton("Anivox||News⛩️✅", url=ANINEWS)
    btn3 = types.InlineKeyboardButton("Kerakli - Kanal✅", url=KANAL)
    btn_check = types.InlineKeyboardButton("🔄 Tekshirish 🔄", callback_data=f"check_{param}")
    markup.add(btn1, btn2, btn3, btn_check)
    bot.send_message(user_id, "❗ Videoni ko'rish uchun quyidagi kanallarga obuna bo'ling:", reply_markup=markup)
            ...
            for i in range(1, 14):
                ...
                if key in animelar:
                    bot.send_video(
                        ...
                        supports_streaming=True  # Streaming qo'shildi
                    )
                    time.sleep(1.2)
        else:
...
def get_id(message):
    if message.content_type == 'video':
        bot.send_message(message.chat.id, f"Video ID: `{message.video.file_id}`", parse_mode="Markdown")
    elif message.content_type == 'photo':
        bot.send_message(message.chat.id, f"Photo ID: `{message.photo[-1].file_id}`", parse_mode="Markdown")
