...
# FAQAT ISHLAYDIGAN 2 TA KANAL (Bot admin bo'lishi shart!)
...
# --- INLINE TUGMALAR (MAXIMAL UZUN) ---
def show_sub_menu(user_id, param="search"):
    markup = types.InlineKeyboardMarkup(row_width=1)
    
    # Uzun va simmetrik tugmalar (probellar bilan cho'zilgan)
...
def process_search(message):
    ...
    msg = bot.send_message(message.chat.id, "Yana qidiramizmi?", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add("❌ Bekor qilish"))
    bot.register_next_step_handler(msg, process_search)
...
def callback_check(call):
    ...
    if check_sub(user_id):
        ...
        if param in animelar:
            ...
            bot.send_message(user_id, "✅ Obuna tasdiqlandi! Endi botdan foydalana olasiz.", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).row("🔍 Anime izlash", "📜 Animelar ro'yxati").row("💵 Reklama va Hamkorlik"))
    else:
