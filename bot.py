...
# --- RO'YXAT ---
@bot.message_handler(func=lambda m: "ro'yxati" in m.text or "Animelar" in m.text and len(m.text) < 25)
...
def search_mode(message):
    if check_sub(message.from_user.id):
        msg = bot.send_message(message.chat.id, "🔍 Anime nomini va qismini yozing:\n*(Masalan: 'Jahannam 1')*", reply_markup=cancel_keyboard(), parse_mode="Markdown")
...
def process_search(message):
    ...
    if not found:
        bot.send_message(message.chat.id, "❌ Topilmadi. Masalan: 'Jahannam 1' deb yozing.")
