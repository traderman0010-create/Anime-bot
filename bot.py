...
def show_sub_menu(user_id, action_type="search"):
    ...
    btn1 = types.InlineKeyboardButton("Animelar || Uzbek Tilida⛩️✅", url="https://t.me/Anivoxuz")
    ...
    btn_check = types.InlineKeyboardButton("🔄 Obunani tekshirish 🔄", callback_data=f"check_sub_{action_type}")
    markup.add(btn1, btn2, btn3, btn_check)
    bot.send_message(user_id, "🚀 **Botdan foydalanish uchun barcha kanallarga obuna bo'ling!**\n\nObuna bo'lmaguningizcha bot sizga **animelarni** yubormaydi. 👇", reply_markup=markup, parse_mode="Markdown")
...
def list_handler(message):
    text = (
        ...
        "🔥 Jahannam Jannati: 1-Fasl (1-13)\n"
        "🌀 Jahannam Jannati: 2-Fasl (1-...)\n"
        "🌸 Sening isming (Film)\n\n"
        "🔍 Qidirish uchun **Anime izlash** tugmasidan foydalanib 'Jahannam 1' yoki '2Jahannam 1' deb yozing!"
    )
...
def process_search(message):
    ...
    for k, data in animelar.items():
        if user_input in k.lower() or user_input in data['caption'].lower().replace(" ", ""):
            bot.send_video(message.chat.id, data['id'], thumb=THUMBNAIL, caption=f"🎬 {data['caption']}\n📢 @Anivoxuz", supports_streaming=True)
            found = True
            time.sleep(0.5)

    if not found:
        bot.send_message(message.chat.id, "❌ Anime topilmadi. Masalan: 'Jahannam 1' deb yozib ko'ring.")
...
def callback_check(call):
    if check_sub(call.from_user.id):
        ...
        bot.send_message(call.message.chat.id, "Obuna tasdiqlandi! Anime izlashni boshlang:", reply_markup=main_keyboard())
    else:
