...
# 🎬 ANIME BAZASI (Yangi qismlar qo'shildi)
animelar = {
    # 1-Fasl
...
def list_handler(message):
    list_text = (
        ...
        "🔥 **Jahannam Jannati:**\n"
        "• 1-Fasl (1-13 qismlar)\n"
        "• 2-Fasl (1, 2, 3, 4-qismlar qo'shildi)\n\n"
        "🌸 **To'liq metrajli:**\n"
        "• Sening isming (Film)\n\n"
...
def callback_check(call):
    ...
    if check_sub(user_id):
        ...
        if param == "all_jahannam":
            for i in range(1, 14):
                k = f"jahannam{i}"
                bot.send_video(user_id, animelar[k]['id'], caption=f"🎬 {animelar[k]['caption']}", supports_streaming=True)
                time.sleep(0.5)
        elif param in animelar:
            bot.send_video(user_id, animelar[param]['id'], caption=f"🎬 {animelar[param]['caption']}", supports_streaming=True)
        else:
            bot.send_message(user_id, "✅ Tasdiqlandi!", reply_markup=main_keyboard())
    else:
