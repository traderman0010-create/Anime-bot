...
def show_sub_menu(user_id, param="search"):
    ...
    # Perfectionist uslubidagi uzun va simmetrik tugmalar
...
def start_cmd(message):
    ...
    # Kanaldan link orqali kelsa (masalan: ?start=jahannam1)
    if len(text) > 1:
        param = text[1]
...
# --- QIDIRUV (FAQAT SHUNDA OBUNA SO'RAYDI) ---
@bot.message_handler(func=lambda m: "izlash" in m.text)
def search_mode(message):
    user_id = message.from_user.id
    if check_sub(user_id):        msg = bot.send_message(message.chat.id, "🔍 Anime nomini yozing:", reply_markup=cancel_keyboard())
...
def callback_check(call):
    ...
    if check_sub(user_id):
        ...
        if param in animelar:
            ...
            bot.send_message(user_id, "✅ Obuna tasdiqlandi! Endi botdan foydalanishingiz mumkin.", reply_markup=main_keyboard())
    else:
