...
# KANALLAR (ID-larni tekshirib chiqing, ayniqsa 3-sini)
...
def check_sub(user_id):
    for channel in CHANNELS:
        try:
            ...
            # Agar bot kanal admini bo'lmasa yoki kanal topilmasa bu yerga tushadi
            print(f"Xato: {channel} da bot admin emas yoki ID noto'g'ri!")
            return False
    return True
...
def show_sub_menu(user_id, param="search"):
    ...
    # Tugmalarni maksimal darajada uzun qilish uchun probellar
    t1 = "Anivox          ||          Animelar ⛩️✅"
    t2 = "Anivox          ||          AnimeNews ❗✅"
    t3 = "Anivox          ||          zayafka ♻️⛩️"
    t4 = "🔄 OBUNANI TASDIQLASH 🔄"
    
    markup.add(types.InlineKeyboardButton(t1, url=MAIN_CHANNEL))
    markup.add(types.InlineKeyboardButton(t2, url=NEWS_CHANNEL))
    markup.add(types.InlineKeyboardButton(t3, url=ZAYAFKA_CHANNEL))
    markup.add(types.InlineKeyboardButton(t4, callback_data=f"check_sub_{param}"))
...
def process_search(message):
    if message.text == "❌ Bekor qilish":
        bot.send_message(message.chat.id, "Bekor qilindi.", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).row("🔍 Anime izlash", "📜 Animelar ro'yxati").row("💵 Reklama va Hamkorlik"))
        return
    
    # Sodda qidiruv mantiqi
