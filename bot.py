...
def search_mode(message):
    if check_sub(message.from_user.id):
        msg = bot.send_message(message.chat.id, "🔍 Anime nomini yoki qismini yozing (Masalan: Jahannam 1)\n\nTo'xtatish uchun tugmani bosing 👇", reply_markup=cancel_keyboard())
...
def process_search(message):
    ...
    if message.text == "❌Qidiruvni bekor qilish" or message.text == "/start":
        bot.send_message(message.chat.id, "Qidiruv to'xtatildi. Asosiy menyu:", reply_markup=main_keyboard())
        return

    query = message.text.lower().replace(" ", "") # "Jahannam 1" -> "jahannam1"
    found = False
    
    for k, data in animelar.items():
        # Ham caption ichidan, ham qisqartirilgan kalit so'zdan qidiradi
        if query in k.lower() or query in data['caption'].lower().replace(" ", ""):
            bot.send_video(message.chat.id, data['id'], thumb=THUMBNAIL, caption=f"🎬 {data['caption']}\n📢 @Anivoxuz")
            found = True
            time.sleep(0.5)

    if not found:
        bot.send_message(message.chat.id, "❌ Hech narsa topilmadi. Boshqa nom yozib ko'ring.")
    
    # QIDIRUVNI TO'XTATMASLIK: Yana bir marta o'z-o'ziga register qiladi
...
def callback_check(call):
    ...
    if check_sub(u):
        ...
        if p == "search": 
            bot.send_message(u, "Endi qidirishingiz mumkin!", reply_markup=main_keyboard())
        else:
            # Ssilka orqali kelgan parametrlarni jo'natish mantiqi (ixtiyoriy qo'shish mumkin)
            bot.send_message(u, "Obuna tasdiqlandi. Menyu:", reply_markup=main_keyboard())
    else:
