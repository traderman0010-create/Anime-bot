...
# --- BOT SOZLAMALARI (TOKEN ENDI XAVFSIZ) ---
TOKEN = '7747703136:AAFbeOSwtodo7r_TS_sefsso1_naLSm2iVg'  # Siz env ga o'tkazing
bot = telebot.TeleBot(TOKEN)

# FAQAT ISHLAYDIGAN 2 TA KANAL
...
# --- ANIME BAZASI (TAKRORLANISH TUZATILDI) ---
animelar = {
    ...
    '2jahannam3': {'id': 'BAACAgIAAxkBAAIF_WmuHcoi9DGLB36gpFueo4xMYhDCAAIwlQACx35xSYZSb6aLlf-nOgQ', 'caption': 'Jahannam Jannati [2-Fasl 3-Qism]'},  # 🔧 TUZATILDI: Yosh ID
...
# --- OBUNA TEKSHIRISH (XATOLIK LOG QILINDI) ---
...
# --- KUCHAYTIRILGAN QIDIRUV (REGEX TUZATILDI) ---
def extract_anime_key(text):
    text = text.lower()
    
    # "jahannam 2 5" yoki "2 5" yoki "2-fasl 5-qism" formatlari
    match = re.search(r'(?:jahannam\s*)?(?:2\s*[-]?\s*fasl\s*)?(\d+)[-\s]*(?:qism\s*)?(\d+)?', text)
    
    if match:
        if match.group(2):  # 2 ta raqam: fasl va qism
            fasl = match.group(1)
            qism = match.group(2)
            if fasl == '2':
                return f'2jahannam{qism}'
            else:
                return f'jahannam{qism}'
        else:  # 1 ta raqam
            raqam = match.group(1)
            if raqam == '2':
                return '2jahannam1'  # default 1-qism
            elif raqam == '1':
                return 'jahannam1'
            else:
                return f'jahannam{raqam}'
    
    # Eski format bilan moslash: "2jahannam5" kabi
    direct_match = re.search(r'(2?jahannam)(\d+)', text)
    if direct_match:
...
# --- QIDIRUV (YANGI REGEX BILAN) ---
@bot.message_handler(func=lambda m: "izlash" in m.text)
def search_mode(message):
    if check_sub(message.from_user.id):
        msg = bot.send_message(message.chat.id, "🔍 Anime nomini yozing (masalan: Jahannam 2 5):", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add("❌ Bekor qilish"))
...
def process_search(message):
    if message.text == "❌ Bekor qilish" or message.text == "/start":
        ...
        markup.row(types.KeyboardButton("🔍 Anime izlash"), types.KeyboardButton("📜 Animelar ro'yxati"))
        markup.row(types.KeyboardButton("💵 Reklama va Hamkorlik"))
        bot.send_message(message.chat.id, "Qidiruv to'xtatildi.", reply_markup=markup)
        return

    user_input = message.text.lower()
    search_key = extract_anime_key(user_input)
    
    if search_key and search_key in animelar:
        data = animelar[search_key]
        bot.send_video(message.chat.id, data['id'], thumb=THUMBNAIL, caption=f"🎬 {data['caption']}\n📢 @Anivoxuz", supports_streaming=True)
    else:
        bot.send_message(message.chat.id, "❌ Topilmadi. Masalan: 'jahannam 2 3' yoki '2 4' deb yozing.")
    
    msg = bot.send_message(message.chat.id, "Yana qidiramizmi? ('❌ Bekor qilish' uchun)", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add("❌ Bekor qilish"))
    bot.register_next_step_handler(msg, process_search)
...
def callback_check(call):
    ...
    if check_sub(user_id):
        ...
        if param in animelar:
            ...
            markup.row(types.KeyboardButton("🔍 Anime izlash"), types.KeyboardButton("📜 Animelar ro'yxati"))
            markup.row(types.KeyboardButton("💵 Reklama va Hamkorlik"))
            bot.send_message(user_id, "✅ Obuna tasdiqlandi! Endi botdan foydalana olasiz.", reply_markup=markup)
    else:
...
def list_handler(message):
    list_text = "⛩️ **Animelar ro'yxati:**\n\n• Jahannam Jannati (1-13, 2-fasl 1-4 qism)\n• Sening isming (Film)"
