...
def get_id(message):
    if message.content_type == 'video':
        ...
        bot.send_message(message.photo[-1].file_id, f"`{message.photo[-1].file_id}`", parse_mode="Markdown")

if __name__ == "__main__":
    bot.remove_webhook()
    keep_alive()
    print("Bot yangilangan ro'yxat bilan ishlamoqda...")
    while True:
        try:
